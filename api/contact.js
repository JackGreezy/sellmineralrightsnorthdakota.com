const SITE = {
  businessName: "Sell Mineral Rights North Dakota",
  businessEmail: "northdakota@sellmineralrightsnorthdakota.com",
  siteUrl: "https://sellmineralrightsnorthdakota.com"
};
...
function leadFrom(body, req) {
  return {
    name: clean(body.owner_name),
    email: clean(body.email),
    phone: clean(body.phone),
    propertyLocation: clean(body.county_state),
    details: clean(body.property_details),
...
function invalid(lead) {
  if (!lead.name || !lead.email || !lead.propertyLocation || !lead.details) {
...
async function sendEmail(to, lead) {
  ...
  const rows = [
    ...
    ["Property and sale question", lead.details],
    ["Source", lead.source]
  ];
  ...
  const response = await fetch("https://api.sendgrid.com/v3/mail/send", {
    ...
    body: JSON.stringify({
      ...
      subject: `North Dakota mineral review request: ${lead.propertyLocation}`,
      content: [
        { type: "text/plain", value: text },
        { type: "text/html", value: html }
      ],
      categories: ["mineral-rights-lead", "sellmineralrightsnorthdakota-com"]
    })
  });
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 500, {
      ok: false,
      error: "The mineral review could not be submitted. Please call or email the review desk."
    });
  }
};
